...
public class ProductService {
  ...
  public ProductDTO editProduct(ProductDTO productDTO, int categoryId) {
    ...
    optionalProduct
        .get()
        .getCategories()
        .forEach(category -> category.getProducts().remove(optionalProduct.get()));
    ...
    for (int i = 0; i < productDTO.getCategoryIdList().size(); i++) {
      Optional<Category> category =
          categoryRepository.findById(productDTO.getCategoryIdList().get(i));
      category.get().getProducts().add(product);
    }
  ...
  public Boolean deleteProduct(int id) {
    ...
    productOptional.get().getCategories().stream()
        .forEach(category -> category.getProducts().remove(productOptional.get()));
  ...
  public ProductDTO getProductById(int id) {
    ...
    productOptional.get().getCategories().stream()
        .forEach(category -> categories.add(category.getId()));
  ...
  public Page<ProductDTO> getFindPage(Pageable pageable) {
    ...
    Page<ProductDTO> productDTOPage =
        new PageImpl(productDTOList, pageable, productPage.getTotalElements());
